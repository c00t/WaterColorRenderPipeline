...
namespace UnityEngine.Rendering.LWRP
{
    ...
    public sealed partial class LightweightRenderPipeline : RenderPipeline
    {
        ...
        //when returned by RenderPiepelineAssets
        public LightweightRenderPipeline(LightweightRenderPipelineAsset asset)
        {
            asset.m_RendererSetup = new ForwardRendererSetup(); //just the default construction function
            renderer = new ScriptableRenderer(asset); //持有一些临时创建的material
            ...
            Shader.globalRenderPipeline = "LightweightPipeline"; // it's a tag in shader
        ...
        {
            BeginFrameRendering(cameras); // dll function
        ...
        public static void RenderSingleCamera(LightweightRenderPipeline pipelineInstance, ScriptableRenderContext context, Camera camera, IRendererSetup setup = null)
        {
            ...
            {
                ScriptableRenderer renderer = pipelineInstance.renderer; // setup in the construction function
                var settings = asset;
                InitializeCameraData(settings, camera, out var cameraData);
                SetupPerCameraShaderConstants(cameraData);//对于每个camera在渲染的时候都走一遍这个流程，当不存在时创建这个global vector,当存在时重新定义这个global vector
                ...
                renderer.Clear();//把所有的Pass 数据都清空掉，注意这个是每帧调用的

                var rendererSetup = setup ?? settings.rendererSetup;//这就说明可以在camera上挂一个组件来override这个setup
                rendererSetup.Setup(renderer, ref renderingData); //the Forward Render Setup
                renderer.Execute(context, ref renderingData);//这里需要注意的是所有的execute都必须传入一个context
            }

            context.ExecuteCommandBuffer(cmd);
            CommandBufferPool.Release(cmd);
            context.Submit();//提交渲染
            //TODO: 好像PostProcess v2也是有源码的，可以看看怎么写部分效果的，然后移植出来
        }
            ...
            int maxVisibleAdditionalLights, int maxPerObjectAdditionalLights, out RenderingData renderingData)
        {
            ...
            InitializeWatercolorData(settings, out renderingData.watercolorData);
            ...
            renderingData.enableWC = settings.enableWaterColor;
        }
        static void InitializeWatercolorData(LightweightRenderPipelineAsset settings,out WatercolorData watercolorData)
        {
            watercolorData.depthRange = settings.DepthRange;
            watercolorData.atmosphereTint = settings.AtmosphereTint;
            watercolorData.atmosphereRange = settings.AtmosphereRange;
            watercolorData.pigmentDensity = settings.PigmentDensityParam;
            watercolorData.edgeDarkenIntensity = settings.EdgeDarkenIntensity;
            watercolorData.edgeDarkenWidth = settings.EdgeDarkenWidth;
            watercolorData.bleedingThreshold = settings.BleedingThreshold;
            watercolorData.drybrushThreshold = settings.DrybrushThreshold;
            watercolorData.gapsOverlapsWidth = settings.GapsOverlapsWidth;
            watercolorData.substrateColor = settings.SubstrateColor;
            watercolorData.substrateShading = settings.SubstrateShading;
            watercolorData.substrateLightingDir = settings.SubstrateLightingDir;
            watercolorData.substrateLightingTilt = settings.SubstrateLightingTilt;
            watercolorData.substrateScale = settings.SubstrateScale;
            watercolorData.substrateRoughness = settings.SubstrateRoughness;
            watercolorData.substrateDistortion = settings.SubstrateDistortion;
            watercolorData.postprocessSaturation = settings.PostprocessSaturation;
            watercolorData.postprocessContrast = settings.PostprocessContrast;
            watercolorData.postprocessBrightness = settings.PostprocessBrightness;
        }
