...
namespace UnityEngine.Experimental.Rendering.LWRP
{
    ...
    {
        private static class MainLightShadowConstantBuffer // 啊啊，这个可以在其他地方进行更改
        {
        ...
        {
            RegisterShaderPassName("ShadowCaster");//这个对应的是每个material里的pass name?是 light mode

            m_MainLightShadowMatrices = new Matrix4x4[k_MaxCascades + 1];
            m_CascadeSlices = new ShadowSliceData[k_MaxCascades];
            m_CascadeSplitDistances = new Vector4[k_MaxCascades];
            //然后这里注册一下shader里对应的各种非导出变量
        ...
        public bool Setup(RenderTargetHandle destination, ref RenderingData renderingData)//这里只对数据进行了计算而不是写入到buffer里
        {
            Clear();
        ...
        //注意在clear里应该1、清除这个pass里需要的texture
        void Clear()
        {
            m_MainLightShadowmapTexture = null;
        ...
        void RenderMainLightCascadeShadowmap(ref ScriptableRenderContext context, ref CullingResults cullResults, ref LightData lightData, ref ShadowData shadowData)
        {
            ...
            //command buffer 在确实需要的时候再从pool 中去获取
            ...
            //要注意的是，在每一帧执行的时候，都会事先disable这个keyword，这就说明了这个keyword是全局的
            //而且这里还可以根据设定的条件进行更改，比如asset settings 里设置的东西
