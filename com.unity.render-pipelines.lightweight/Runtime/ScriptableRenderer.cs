...
namespace UnityEngine.Experimental.Rendering.LWRP
{
    public sealed class ScriptableRenderer
    {
        ...
        readonly Texture m_Substrate;

        public ScriptableRenderer(LightweightRenderPipelineAsset pipelineAsset)
        {
            if (pipelineAsset == null)
                throw new ArgumentNullException("pipelineAsset null");
            ...
            m_Materials = new[]
            {
                ...
                CoreUtils.CreateEngineMaterial(pipelineAsset.CreateLinearDepth),
                CoreUtils.CreateEngineMaterial(pipelineAsset.AdjustLoad),
                CoreUtils.CreateEngineMaterial(pipelineAsset.EdgeDetection),
                CoreUtils.CreateEngineMaterial(pipelineAsset.PigmentDensity),
                CoreUtils.CreateEngineMaterial(pipelineAsset.Separable),
                CoreUtils.CreateEngineMaterial(pipelineAsset.Blend),
                CoreUtils.CreateEngineMaterial(pipelineAsset.EdgeManipulation),
                CoreUtils.CreateEngineMaterial(pipelineAsset.GapsOverlaps),
                CoreUtils.CreateEngineMaterial(pipelineAsset.PigmentApplication),
                CoreUtils.CreateEngineMaterial(pipelineAsset.Substrate),
            };
            m_Substrate = pipelineAsset.SubstrateBack;
            postProcessingContext = new PostProcessRenderContext();// it's a sealed class, so ...
        }
        ...
        public Texture GetSubstrateBack()
        {
            return m_Substrate;
        }
        ...
        // do all frame clean up ， 这里需要注意的是，我的texture里有一些部分是不需要cleanup的
