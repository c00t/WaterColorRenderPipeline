...
namespace UnityEditor.ShaderGraph
{
    ...
    static class GraphUtil
    {
        ...
        public static void GenerateWaterColorApplicationVertexInputs(ShaderGraphRequirements graphRequiements, ShaderStringBuilder vertexInputs)
        {
            vertexInputs.AppendLine("struct GraphVertexInput");
            using (vertexInputs.BlockSemicolonScope())
            {
                ...
                vertexInputs.AppendLine("float3 normal : NORMAL;");
                vertexInputs.AppendLine("float4 tangent : TANGENT;");
                vertexInputs.AppendLine("float4 texcoord0 : TEXCOORD0;");
                foreach (var channel in graphRequiements.requiresMeshUVs.Distinct())
                {
                    if((int)channel == 1)
                    {
                ...
                vertexInputs.AppendLine("float4 controlSetA : COLOR;");
                vertexInputs.AppendLine("float4 controlSetB : TEXCOORD2;");
                vertexInputs.AppendLine("float4 controlSetC : TEXCOORD3;");
                vertexInputs.AppendLine("UNITY_VERTEX_INPUT_INSTANCE_ID");
            }
        }
